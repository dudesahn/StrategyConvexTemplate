...
@pytest.fixture(scope="session")
def contract_name(StrategyConvexOriginal4Pools):
    contract_name = StrategyConvexOriginal4Pools
    yield contract_name
...
@pytest.fixture(scope="session")
def try_blocks():
    try_blocks = False  # True for sUSD, but only need to use for Curve strategy
    yield try_blocks
...
def sleep_time():
    ...
    hours_to_sleep = 4  # 12 works for sUSD, 6 for yUSD, 4 for yBUSD
