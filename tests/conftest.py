...
@pytest.fixture(scope="session")
def contract_name(StrategyConvexEthPoolsClonable):
    contract_name = StrategyConvexEthPoolsClonable
    yield contract_name
...
@pytest.fixture(scope="session")
def has_rewards():
    has_rewards = False  # false for all ETH
    yield has_rewards
...
def sleep_time():
    ...
    hours_to_sleep = 3  # 3 hours for all ETH
